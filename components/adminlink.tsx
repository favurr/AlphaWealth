  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background flex items-center justify-center px-4">
      ...
      {floatingGears.map((_, i) => (
        <motion.div
          key={i}
          className="absolute h-32 w-32 rounded-full border-2 border-dashed opacity-40 border-green-500"
          initial={{
      ...
      >
        ...
        <div className="relative mx-auto mb-6 h-24 w-24">
          <motion.div
            className="absolute inset-0 rounded-full border-2 border-dashed opacity-40 border-green-500"
            animate={{ rotate: 360 }}
            transition={{ repeat: Infinity, duration: 10, ease: "linear" }}
          />
          <motion.div
            className="absolute inset-4 rounded-full border opacity-30 border-green-500"
            animate={{ rotate: -360 }}
        ...
        <div className="mt-8 flex items-center justify-center gap-3">
          <Link href="/admin">
            <Button className="inline-flex items-center justify-center rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow transition hover:opacity-90">
              <ShieldUser className="mr-2" /> Admin Panel
            </Button>
          </Link>
